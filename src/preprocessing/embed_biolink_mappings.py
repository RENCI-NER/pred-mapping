...
async def embed_biolink_predicates(infile, outfile, use_lowercase=False):
    ...
    for predicate, text_list in tqdm(data.items(), position=0, desc="Biolink predicates Embedding"):
        predicate_uri = "biolink:" + predicate.strip().replace(" ", "_")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-m", "--mappings", default="/Users/olawumiolasunkanmi/PycharmProjects/LitCoin_RE_SapBert/sapbert/training_data/RE/all_biolink_mappings.json",
                        help="Input biolink mapping file")
    parser.add_argument("-q", "--qualified_mappings", default="qualified_predicate_mappings.json",
                        help="Input biolink mapping file")
    parser.add_argument("-e", "--embeddings", default="all_biolink_mapped_vectors.json",
                        help="Output biolink embedding file")
